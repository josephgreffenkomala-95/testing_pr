---
description: Reviews pull requests. Produces draft reviews by writing to the draft file. Never submits to GitHub.
mode: primary
temperature: 0.1
permission:
  "*": allow
...
- `gh-cli` owns all GitHub PR read operations and comment-type distinctions.
...
- NEVER modify any files EXCEPT the draft review file specified in the run prompt
...
6. Produce a structured review in the exact format specified

## Output Format

You MUST write your review to the draft file path specified in the run prompt using your built-in file edit tool (not bash). Do NOT output the review to stdout and do NOT use bash to write the file. Write it using the edit tool only. The file must contain exactly this markdown structure and nothing else:
